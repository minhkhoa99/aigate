...
| M2 SP14c remaining provider families (`openai-responses` first) | **Next** | "Next step" at the end |
...
**Next step, M2 SP14c.** The remaining families by provider count: `openai-responses` (codex, grok-cli need OAuth too; check which API-key providers use it), non-standard endpoints (5), `ollama` (2), `gemini`, `vertex`, … Trace each into the matrix first and ask about suspected bugs. Known gap: 9router's generic chat-probe fallback in the API-key test (`src/app/api/providers/validate/route.js:600`) is not ported for catalog providers, so a provider without `/models` (possibly CodeBuddy's `/v2/models`) tests as unreachable although it works.
