...
| M2 SP15 client-facing protocol adapters (the 13 formats of spec §9) | **Next** | "Next step" at the end |
...
**Next step, M2 SP15.** Spec §9: "Protocol adapter đủ 13 format" — the client-facing protocols beyond OpenAI Chat (for example Anthropic Messages `/v1/messages`, OpenAI Responses `/v1/responses`, Gemini `generateContent`, and the other 9router source formats). The API-key provider families of SP14 are done (59 connectable); the rest of the catalog needs OAuth (SP16), media/search lanes (SP22/SP23), web sessions, or is not ported (qoder). Trace 9router's source-format detection and translators into the matrix first (`routing.source-format-detection`, `routing.request-translation`), agree the SP15 scope with the user, and ask about suspected bugs. Open question for SP22: nanobanana is image-only but still connectable. Known gap: 9router's generic chat-probe fallback in the API-key test (`src/app/api/providers/validate/route.js:600`) is not ported for catalog providers without a `chatProbe`.
