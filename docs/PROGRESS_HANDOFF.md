...
**SP0.2 RED baseline is recorded** in `docs/superpowers/skill-tests/2026-09-25-writing-lean-bounded-red.md`: three independent scenarios, with one clear failure (usage query still loads all rows for one account). Next, write `writing-lean-bounded-code` and its three references for SP0.3, targeting that observed failure; run GREEN and micro-tests before connecting it in `CLAUDE.md`. Then SP0.4-SP0.6 and SP1.
