...
| M2 SP14g per-account URLs (azure, cloudflare-ai), clinepass envelope, then commandcode (NDJSON) | **Next** | "Next step" at the end |
...
**Next step, M2 SP14g.** Traced in SP14f, not yet in the matrix:
- **azure** (catalog chatUrl null): per-connection `azureEndpoint`, `deployment` (else the request model), `apiVersion` (default `2024-10-01-preview`), optional `organization`; `POST <endpoint>/openai/deployments/<deployment>/chat/completions?api-version=…` with an `api-key` header; body sent unchanged. Suspected bugs: validation passes 404/400 (wrong deployment), the form forces Organization, an unset endpoint falls back to api.openai.com, unencoded path segments.
- **cloudflare-ai**: per-connection `accountId` in `…/accounts/{accountId}/ai/v1/chat/completions`, Bearer; content arrays flattened to text (images dropped); test valid unless 401/403/404.
- **clinepass**: extra headers (HTTP-Referer, X-Title, client identity headers), non-stream `{ success: true, data }` envelope unwrapped; Test always fails in 9router.
- **commandcode**: NDJSON AI SDK v5 events, forced stream; several 9router stream bugs (lines lost in the peek phase, no `[DONE]`, error message lost, cut-off treated as complete).
Trace each into the matrix first and ask about suspected bugs. Connection data needs new per-connection fields (like SP14d's `base_url`). Open question for SP22: nanobanana is image-only but still connectable. Known gap: 9router's generic chat-probe fallback in the API-key test (`src/app/api/providers/validate/route.js:600`) is not ported for catalog providers.
