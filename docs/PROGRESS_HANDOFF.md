...
| M2 SP14e `gemini` adapter, then vertex, non-standard endpoints, commandcode, per-account URLs | **Next** | "Next step" at the end |
...
**Next step, M2 SP14e.** `gemini` (API key, `x-goog-api-key`, generateContent/streamGenerateContent translators), then `vertex` / `vertex-partner` (service-account auth), qoder/qoder-cn (their own SSE agent endpoint), `commandcode` (NDJSON), and the per-account URLs (azure, cloudflare-ai, now possible through `connectionBaseUrl`-style connection data). Trace each into the matrix first and ask about suspected bugs. Open question for SP22: nanobanana is image-only but still connectable. Known gap: 9router's generic chat-probe fallback in the API-key test (`src/app/api/providers/validate/route.js:600`) is not ported for catalog providers, so a provider without `/models` (possibly CodeBuddy's `/v2/models`) tests as unreachable although it works.
