...
| M2 SP15b OpenAI Responses client protocol (`/v1/responses`), then SP15c Gemini (`/v1beta/models/*`) | **Next** | "Next step" at the end |
...
**Next step, M2 SP15b.** OpenAI Responses clients (`POST /v1/responses`, `/v1/responses/compact`, the `/responses` and `/codex/*` aliases) through the same lane. Traced in SP15: 9router's `translator/request/openai-responses.js` (input items, instructions, tools incl. custom tools, reasoning, `previous_response_id` passed through unresolved) and `translator/response/openai-responses.js` (the response.* event sequence; colliding output_index; `response.completed` without output; length stops as completed; errors as `[Error]` text). Write the matrix entries, then ask about those suspected bugs (the OpenAI error shape and non-stream shape decisions from SP15 already apply). SP15c after it: Gemini `/v1beta` (9router's route is text-only and drops tools). Open question for SP22: nanobanana is image-only but still connectable. Known gap: 9router's generic chat-probe fallback in the API-key test is not ported for catalog providers without a `chatProbe`.
