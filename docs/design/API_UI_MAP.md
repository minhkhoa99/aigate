...
| SP15 Anthropic Messages client protocol (`POST /v1/messages`, `/v1/messages/count_tokens`) | `/gateway/endpoint`: the Base URL panel now names OpenAI and Anthropic, says Anthropic clients call `/v1/messages` with `x-api-key`, and adds copy fields for Claude Code (`ANTHROPIC_BASE_URL` = this origin, `ANTHROPIC_API_KEY`) and an Anthropic curl test. API clients (not the dashboard) read its errors in the lane's OpenAI shape, and a mid-stream failure as `event: error`. `contracts/protocol-anthropic.md`. |
