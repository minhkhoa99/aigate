...
| `POST /api/connections` | same | `Connections` → Add connection modal (the connectable providers, 52 since SP14d; a Host field and optional key for Ollama Local; a query `?provider=` preselects or shows the reason); `PROVIDER_NOT_SUPPORTED` toasts the server's reason | `useCreateConnection` | Wired: save, then test at once |
