...
import { useConnections, useCreateNode, useDeleteNode, useProviderNodes, useUpdateNode, type ProviderNode } from "./api";
...
function NodeForm({ node }: { node?: ProviderNode }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const fields = { name: text(event.currentTarget, "name"), prefix: text(event.currentTarget, "prefix"), baseUrl: text(event.currentTarget, "baseUrl") };
  ...
  return <form onSubmit={submit}><div className="stack">
    ...
    <Field label="Prefix" hint={'Requests name a model as "<prefix>/<model>", e.g. local/llama-3. Lowercase letters, digits, dashes.'}><Input name="prefix" required maxLength={32} defaultValue={node?.prefix} placeholder="local" /></Field>
    ...
    <Field label="Base URL" hint="The URL before /chat/completions. https, or http to this machine only."><Input name="baseUrl" required maxLength={2048} defaultValue={node?.baseUrl} placeholder="https://api.example.com/v1" /></Field>
