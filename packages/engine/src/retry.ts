...
function validate({ maxAttempts, baseDelayMs, maxDelayMs }: RetryOptions): void {
  ...
  if (!Number.isFinite(baseDelayMs) || baseDelayMs < 0) throw new RangeError("baseDelayMs must be a finite number >= 0");
  if (!Number.isFinite(maxDelayMs) || maxDelayMs < baseDelayMs) throw new RangeError("maxDelayMs must be finite and >= baseDelayMs");
}
...
export async function withRetry<T>(operation: (attempt: number, signal: AbortSignal) => Promise<T>, options: RetryOptions): Promise<T> {
  validate(options);
