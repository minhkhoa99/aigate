...
export function resolveCapabilities(model, modelId) {
    if (model)
        return model.capabilities;
    return looksLikeVisionModel(modelId) ? { ...DEFAULT_CAPABILITIES, vision: true } : DEFAULT_CAPABILITIES;
}
function partNeeds(part, needs, pending) {
    switch (part.type) {
        case "image":
            needs.add("vision");
            break;
        case "audio":
            needs.add("audioInput");
            break;
        case "video":
            needs.add("videoInput");
            break;
        case "file":
...
export function detectRequiredCapabilities(request) {
    const needs = new Set();
    if (request.tools && request.tools.length > 0)
        needs.add("tools");
    if (request.reasoning)
        needs.add("reasoning");
    const pending = [...(request.system ?? []), ...request.messages.flatMap((message) => message.content)];
...
export function assertModelSupports(request, providerId, model, modelId) {
    ...
    if (missing.length > 0)
        throw new EngineError("MODEL_UNAVAILABLE", `${target} does not support: ${missing.join(", ")}`, { target, missing });
