...
Read `docs/PROGRESS_HANDOFF.md` before resuming work. M-1 discovery is complete against the read-only 9Router reference at `.reference/9router` commit `39e36d3d`. M0 SP0–SP2 and M1 SP5–SP11 are done (M0 SP3/SP4 are not built yet); the handoff has the current step.
